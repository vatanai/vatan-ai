# AGENTS.md — پنل مدیریت وطن استودیو (Admin Dashboard)
...
## دیپلوی و پوش (Deploy) — کلودیوا

سایت اصلی روی سرویس `vatanai-laravel-cloudiva` در `Cloudiva` با دامنهٔ `aivatan.com` اجرا می‌شود. دیپلوی باید از همین پروژه و با `Cloudiva CLI` انجام شود؛ به مسیر استقرار دیگری دست نزن.
...
# ۱. کامیت و پوش شاخهٔ استقرار
git add -A
git commit -m "توضیح تغییرات"
git push origin crm-integration

# ۲. دیپلوی همین پروژه به سرویس اصلی کلودیوا
npx -y @cloudiva.net/cli deploy --service vatanai-laravel-cloudiva

# ۳. بعد از موفق شدن استقرار، از کنسول تازهٔ همان سرویس اجرا کن:
php artisan migrate --force
```

در صورت استفاده از استقرار خودکار `GitHub`، فقط پوش شاخهٔ `crm-integration` کافی است؛ سپس وضعیت استقرار را در پنل `Cloudiva` بررسی کن. کش‌های `Laravel` در فرایند build سرویس بازسازی می‌شوند و نیازی به پاک‌سازی دستی ندارند.
...
- وضعیت استقرار و لاگ‌ها را در تب «استقرارها» و «لاگ‌ها»ی سرویس `vatanai-laravel-cloudiva` بررسی کن.
- برای حجم‌های کمتر از ۱۰۰ مگابایت از `Cloudiva CLI` استفاده کن؛ برای حجم بیشتر، `FTP` سرویس مناسب‌تر است.

### خطای Permission denied بعد از دیپلوی (فایل config یا هر فایل php)

اگه بعد از دیپلوی، سایت با `Warning: require(...): Failed to open stream: Permission denied` بالا نیومد، یعنی یک یا چند فایل روی مک با پرمیشن بسته (600 — فقط خواندنی برای مالک) ذخیره شدن. `Cloudiva CLI` پرمیشن فایل‌ها رو عیناً به سرور می‌بره و چون PHP روی سرور با یوزر دیگه‌ای اجرا می‌شه، نمی‌تونه فایل رو بخونه. (خطای بعدیش مثل `Class "view" does not exist` فقط عارضه‌ی همینه، نه مشکل جدا.)
