...
<div class="pricing-page bg-[#0a0a0c] text-white min-h-screen py-12 px-4 md:px-8 overflow-x-hidden" dir="rtl">
    ...
    <main class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch mb-20">
        ...
        @forelse($plans as $plan)
            ...
            <div class="pricing-card rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 relative bg-[#111116] border border-[#222230] hover:border-[#333345]">
                
                <div>
                    {{-- تصویر کاور داینامیک پلن --}}
                    <div class="w-full h-40 mb-4 overflow-hidden rounded-xl bg-[#16161c] border border-[#222230] flex items-center justify-center">
                        @if($plan->image_path)
                    ...
                    <span class="text-[10px] font-mono text-gray-500 uppercase tracking-wider block mb-1">
                        شناسه پکیج: {{ $plan->plan_code ?? '—' }}
                    </span>
                    
                    {{-- نام پلن --}}
                    <h2 class="text-lg font-bold mb-3 text-white">{{ $plan->name }}</h2>
                    ...
                    <div class="mb-5 flex items-baseline gap-1" dir="ltr">
                        ...
                        @if($planPrice > 0)
                            <span class="text-[10px] text-gray-400 font-sans ml-1">تومان</span>
                        @endif
                    </div>
                    ...
                    <ul class="space-y-3 text-xs text-gray-300">
                        <li class="flex items-center gap-2.5">
                            ...
                            <span class="font-semibold text-white">شارژ آنی پکیج: {{ number_format($planTokens) }} توکن</span>
                        </li>
                        <li class="flex items-center gap-2.5">
                ...
                <div class="mt-8">
                    <form action="{{ route('pricing.fakePayment', ['plan' => !empty($plan->slug) ? $plan->slug : $plan->id]) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full py-3 px-4 rounded-xl text-xs font-bold transition-all bg-white/[0.04] border border-white/[0.08] text-white hover:bg-[#a07af5] hover:text-[#0c0c10] hover:border-[#a07af5] shadow-lg hover:shadow-[#a07af5]/10">
        ...
        @empty
            <div class="col-span-full text-center py-16 text-gray-500 text-xs bg-[#111116] border border-[#222230] rounded-2xl">
                <i class="fa-solid fa-layer-group text-2xl text-gray-600 mb-2 block"></i>
