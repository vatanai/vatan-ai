@php
  ...
  $instance = $instance ?? 'default'; $controlId = 'cw-' . $instance . '-' . $id;
@endphp
@if($type === 'info')
...
<div class="cw-field" data-field="{{ $id }}" data-field-type="{{ $type }}" data-field-credit="{{ (int)($field['credit_cost'] ?? 0) }}" @if(!empty($field['show_if']['field'])) data-show-field="{{ $field['show_if']['field'] }}" data-show-op="{{ $field['show_if']['op'] ?? 'eq' }}" data-show-value="{{ $field['show_if']['value'] ?? '' }}" @endif>
  ...
  @elseif($type === 'select')
    <div class="cw-select-wrap"><select id="{{ $controlId }}" name="fields[{{ $id }}]" class="cw-input">@foreach($options as $option)<option value="{{ $option['value'] }}" data-option-credit="{{ (int)($option['credit'] ?? 0) }}" {{ $value === $option['value'] ? 'selected' : '' }}>{{ $option['label'] }}</option>@endforeach</select><i class="fa-solid fa-chevron-down"></i></div>
  ...
  @elseif(in_array($type, ['strength','slider']))
    <div class="cw-range-wrap"><div><span>کمترین تغییر</span><output>{{ $value }}{{ $field['unit'] ?? '' }}</output><span>بیشترین شباهت</span></div><input id="{{ $controlId }}" name="fields[{{ $id }}]" type="range" min="{{ ($field['min'] ?? '') !== '' ? $field['min'] : 0 }}" max="{{ ($field['max'] ?? '') !== '' ? $field['max'] : 100 }}" step="{{ ($field['step'] ?? '') !== '' ? $field['step'] : 1 }}" value="{{ $value !== '' ? $value : 50 }}" data-range></div>
  @elseif($type === 'color')
    <div class="cw-color-control"><input id="{{ $controlId }}" name="fields[{{ $id }}]" type="color" value="{{ $value ?: '#16594f' }}"><span data-color-value>{{ $value ?: '#16594f' }}</span><i class="fa-solid fa-eyedropper"></i></div>
  @elseif($type === 'switch')
    <input type="hidden" name="fields[{{ $id }}]" value="0"><label class="cw-toggle"><span>{{ $field['label'] }}</span><input id="{{ $controlId }}" name="fields[{{ $id }}]" value="1" type="checkbox" {{ in_array((string)$value, ['1','true'], true) ? 'checked' : '' }}><i></i></label>
  @elseif($type === 'checkbox')
    <input type="hidden" name="fields[{{ $id }}]" value="0"><label class="cw-check"><input id="{{ $controlId }}" name="fields[{{ $id }}]" value="1" type="checkbox"><i class="fa-solid fa-check"></i><span>{{ $field['label'] }}</span></label>
  @elseif($type === 'style_preset')
    <div class="cw-style-grid">@foreach($options as $option)<label><input type="radio" name="fields[{{ $id }}]" value="{{ $option['value'] }}" data-option-credit="{{ (int)($option['credit'] ?? 0) }}" {{ $value === $option['value'] ? 'checked' : '' }}><span>@if($option['image'] ?? null)<img src="{{ $option['image'] }}" alt="">@else<div class="cw-style-placeholder"><i class="fa-solid fa-wand-magic-sparkles"></i></div>@endif<b>{{ $option['label'] }}</b><i class="fa-solid fa-check"></i></span></label>@endforeach</div>
