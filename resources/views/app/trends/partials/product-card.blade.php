@if(!empty($card))
  <a href="{{ $card['link'] }}" class="trends-card" aria-label="{{ $card['name'] }}">
    @if($card['video'])
      <video class="trends-card-media" src="{{ $card['src'] }}" poster="{{ $card['poster'] }}" autoplay muted loop playsinline preload="none"></video>
    @else
      <img class="trends-card-media" src="{{ $card['src'] }}" alt="{{ $card['name'] }}" loading="lazy" decoding="async">
    @endif
