...
<main class="mr-[294px] flex-1 min-h-screen flex flex-col min-w-0 max-[900px]:mr-0">
  ...
  <div class="admin-content orders-page p-6 flex-1 overflow-y-auto max-[768px]:p-[18px] max-[480px]:p-[14px]" id="content" dir="rtl">
    ...
    <div class="order-panel"><div class="orders-table-wrap"><table class="orders-table"><thead><tr><th>سفارش</th><th>کاربر</th><th>محصول</th><th>وضعیت</th><th>اعتبار سفارش</th><th>بازپرداخت</th><th>زمان</th><th>جزئیات</th></tr></thead><tbody>
      @forelse($orders as $order)<tr><td data-label="سفارش"><span class="order-number">#{{ $order->order_number }}</span></td><td data-label="کاربر"><div class="order-user-name">{{ trim(($order->user?->name ?? 'مهمان').' '.($order->user?->last_name ?? '')) }}</div></td><td data-label="محصول">{{ $order->product?->name_fa ?? 'محصول حذف‌شده' }}</td><td data-label="وضعیت">@include('admin.orders.partials.status-badge',['status'=>$order->payment_status])</td><td data-label="اعتبار سفارش">{{ number_format($order->final_credits) }}</td><td data-label="بازپرداخت"><strong>{{ number_format($order->refunded_credits) }}</strong></td><td data-label="زمان">{{ ($order->refunded_at ?? $order->cancelled_at ?? $order->updated_at)->format('Y/m/d H:i') }}</td><td data-label="جزئیات"><a class="order-btn" href="{{ route('admin.orders.show',$order) }}"><i class="fa-solid fa-eye"></i></a></td></tr>
