...
<main class="mr-[294px] flex-1 min-h-screen flex flex-col min-w-0 max-[900px]:mr-0">
  ...
  <div class="admin-content orders-page p-6 flex-1 overflow-y-auto max-[768px]:p-[18px] max-[480px]:p-[14px]" id="content" dir="rtl">
    ...
    <section class="order-panel"><div class="order-panel-head"><div class="order-panel-title">محصولات برتر</div></div><div class="orders-table-wrap"><table class="orders-table" style="min-width:600px"><thead><tr><th>محصول</th><th>تعداد سفارش</th><th>اعتبار فروش</th><th>سهم از سفارش‌ها</th></tr></thead><tbody>@forelse($products as $product)<tr><td data-label="محصول"><strong>{{ $product['name'] }}</strong></td><td data-label="تعداد سفارش">{{ number_format($product['count']) }}</td><td data-label="اعتبار فروش">{{ number_format($product['credits']) }}</td><td data-label="سهم از سفارش‌ها">{{ $stats['total'] ? round($product['count']*100/$stats['total'],1) : 0 }}٪</td></tr>@empty<tr><td colspan="4"><div class="order-empty">داده‌ای در این بازه وجود ندارد.</div></td></tr>@endforelse</tbody></table></div></section>
  </div>
</main>
@endsection
