  ...
  <div class="admin-main">
    ...
    <header class="admin-header">
      <div class="breadcrumb">
        ...
        <span style="color:var(--text3);font-size:10px;"><i class="fa-solid fa-chevron-left"></i></span>
        <span>محصولات</span><span style="color:var(--text3);font-size:10px;"><i class="fa-solid fa-chevron-left"></i></span><span class="current" id="edit-product-name">ویرایش محصول</span>
      </div>
...
const EDIT_PRODUCT = {
  id: 1,
  name_fa: "عکس حرفه‌ای لینکدین",
  name_en: "LinkedIn Professional Photo",
  slug: "linkedin-professional-photo",
  desc_fa: "تبدیل عکس معمولی به عکس حرفه‌ای برای لینکدین با هوش مصنوعی",
  desc_en: "Transform your casual photo into a professional LinkedIn headshot using AI",
  category: "people",
  subcat: "portrait",
  status: "active",
  tags: ["لینکدین", "پروفایل", "حرفه‌ای"],
  is_featured: true,
  is_new: false,
  is_trending: true,
  primary_model: "black-forest-labs/flux-1.1-pro",
  prompt: "Professional LinkedIn profile photo, formal attire, {{clothing_style}}, clean background, high resolution, 4K",
  pricing_type: "per_credit",
  credit_cost: 5,
};

function prefillEditForm() {
  const d = EDIT_PRODUCT;
  const setVal = (id, v) => { const el = document.getElementById(id); if(el) el.value = v; };
  setVal('name_fa', d.name_fa);
  setVal('name_en', d.name_en);
  setVal('slug', d.slug);
  setVal('desc_fa', d.desc_fa);
  setVal('desc_en', d.desc_en);
  if(document.getElementById('category')) document.getElementById('category').value = d.category;
  if(document.getElementById('status')) document.getElementById('status').value = d.status;
  // tags
  d.tags.forEach(t => addTag(t));
  // prompt
  if(document.getElementById('prompt_template')) document.getElementById('prompt_template').value = d.prompt;
  // credit
  if(document.getElementById('credit_cost')) document.getElementById('credit_cost').value = d.credit_cost;
  // page title
  const el = document.getElementById('edit-product-name');
  if(el) el.textContent = d.name_fa;
  const pt = document.getElementById('page-title-edit');
  if(pt) pt.textContent = 'ویرایش: ' + d.name_fa;
  // view link
  const vl = document.getElementById('view-product-link');
  if(vl) vl.href = '/admin/products/' + d.id;
}

function archiveProduct() {
  if(confirm('آیا از آرشیو کردن این محصول مطمئنید؟')) {
    alert('محصول آرشیو شد.');
    window.location.href = '/admin/products';
  }
}

document.addEventListener('DOMContentLoaded', prefillEditForm);
