      ...
      <div class="grid grid-cols-4 gap-3 p-4 max-[1280px]:grid-cols-3 max-[1000px]:grid-cols-2 max-[650px]:grid-cols-1">
        @forelse($galleryCards as $card)
          <article class="relative rounded-2xl border bg-[var(--page-bg)] border-[var(--border)] overflow-hidden">
            <label class="absolute top-2 right-2 z-10 w-7 h-7 rounded-lg inline-flex items-center justify-center bg-[var(--card-bg)]/90 border border-[var(--border)] cursor-pointer"><input type="checkbox" name="user_ids[]" value="{{ $card['user_id'] }}" class="gallery-card-checkbox accent-[var(--primary)]" aria-label="انتخاب کارت {{ $card['user_name'] }}"></label>
            <a href="{{ route('admin.users.gallery.show', $card['user_id']) }}" class="flex items-center justify-between gap-3 p-3 border-b border-[var(--border)] hover:bg-[var(--primary-l)] no-underline" style="padding-right: 66px;">
