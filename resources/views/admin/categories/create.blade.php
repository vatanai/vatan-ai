  ...
  <main class="flex-1 flex flex-col min-h-screen mr-0 md:mr-[294px]">
    ...
    <div class="admin-content p-6 flex-1 pb-24 overflow-y-auto max-[768px]:p-[18px]" id="content">
      ...
      <div class="mb-6 flex items-center justify-between flex-wrap gap-3">
        ...
        <a href="{{ route('admin.categories.index') }}" class="inline-flex items-center gap-1.5 px-3.5 h-[34px] rounded-lg text-xs font-semibold bg-[#16161c] text-[#a8c4a8] border border-[#222230] transition-all hover:text-white no-underline">
          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="bg-[#16161c] border border-[#222230] rounded-xl p-5">
              <div class="text-xs font-bold text-white mb-4 flex items-center gap-2 pb-3 border-b border-[#222230]">
                <i class="fa-solid fa-folder-plus text-[#a07af5]"></i> مشخصات و اطلاعات هویتی دسته‌بندی
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-semibold text-[#a8c4a8]">نام دسته‌بندی <span class="text-[#f05c5c] mr-0.5">*</span></label>
                  <input type="text" name="name" class="bg-[#111116] border border-[#222230] rounded-lg p-2.5 text-xs text-white outline-none focus:border-[#a07af5] w-full" value="{{ old('name') }}" placeholder="مثلا: مدل‌های متنی یا ابزارهای گرافیکی" required>
                </div>
                
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-semibold text-[#a8c4a8]">اسلاگ (Slug - اختیاری)</label>
                  <input type="text" name="slug" class="bg-[#111116] border border-[#222230] rounded-lg p-2.5 text-xs text-white outline-none focus:border-[#a07af5] w-full text-right" value="{{ old('slug') }}" placeholder="در صورت خالی بودن خودکار تولید می‌شود">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="bg-[#16161c] border border-[#222230] rounded-xl p-5">
              <div class="text-xs font-bold text-white mb-3 flex items-center gap-2 pb-2 border-b border-[#222230]">
                <i class="fa-regular fa-image text-[#a07af5]"></i> تصویر یا لوگوی دسته‌بندی
              </div>
              ...
              <div class="flex flex-col items-center justify-center border border-dashed border-[#333345] bg-[#111116] rounded-xl p-4 transition hover:border-[#a07af5] relative group min-h-[140px]">
                ...
                <div id="upload-placeholder" class="flex flex-col items-center justify-center text-center cursor-pointer">
                  ...
                  <span class="text-[11px] text-[#a8c4a8] font-medium">انتخاب تصویر برای دسته‌بندی</span>
                  <span class="text-[9px] text-gray-600 mt-0.5">فرمت‌های مجاز: jpeg, png, jpg, webp (حداکثر ۲ مگابایت)</span>
                </div>
...
@if($errors->any())
<div id="alert-toast" class="fixed top-6 left-6 z-50 transform translate-x-[-120%] opacity-0 transition-all duration-500 ease-out flex items-start gap-3 bg-[#16161c] border border-[#f05c5c]/40 shadow-2xl shadow-[#0c0c10] p-4 rounded-xl min-w-[340px] max-w-sm text-right" dir="rtl">
    <div class="w-9 h-9 rounded-lg bg-[#f05c5c]/10 flex items-center justify-center text-[#f05c5c] shrink-0 mt-0.5">
        <i class="fa-solid fa-circle-exclamation text-base"></i>
    </div>
    <div class="flex-1">
        <div class="text-xs font-bold text-white mb-1.5">خطا در ثبت اطلاعات</div>
        <ul class="text-[11px] text-gray-400 list-disc pr-4 space-y-1">
            @foreach ($errors->all() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </ul>
    </div>
    <button onclick="closeToast('alert-toast')" class="text-gray-500 hover:text-white transition-colors p-1 cursor-pointer">
