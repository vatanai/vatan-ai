  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="lg:col-span-2 bg-[var(--card-bg)] border border-[var(--border)] rounded-xl p-5 shadow-[var(--shadow-card)]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label class="text-xs font-bold text-[var(--text-main)]">نسبت کاور<select name="cover_ratio" class="mt-2 w-full rounded-lg bg-[var(--input-bg)] border border-[var(--border)] p-2.5 text-[var(--text-main)]"><option value="1:1" @selected(old('cover_ratio', $category?->cover_ratio ?: '1:1') === '1:1')>مربع · ۱:۱</option><option value="2:1" @selected(old('cover_ratio', $category?->cover_ratio) === '2:1')>افقی · ۲:۱</option><option value="1:2" @selected(old('cover_ratio', $category?->cover_ratio) === '1:2')>عمودی · ۱:۲</option></select><small class="block mt-1.5 font-normal text-[var(--text-soft)]">نسبت نمایش تصویر در صفحه دسته‌بندی</small></label>
