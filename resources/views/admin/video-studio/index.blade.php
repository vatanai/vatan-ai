  ...
  <div class="video-studio-page" id="content">
    ...
    <div class="studio-settings" style="margin-bottom:16px">
      <section class="studio-card studio-panel" id="studio-settings">
        ...
        <form id="studio-settings-form" class="studio-form" data-product-id="{{ $selectedProductId ?? '' }}" method="POST" action="{{ route('admin.video-studio.settings.update') }}" enctype="multipart/form-data">
          ...
          @endif
          <div class="studio-field"><label>فونت نوشته‌های ویدیو</label><div class="studio-font-grid" id="font-family">@foreach($fonts as $font)<div class="studio-font-option"><input id="font-{{ $font->slug }}" type="radio" name="font_family" value="{{ $font->slug }}" @checked(($settings->font_family ?? 'Modam') === $font->slug)><label for="font-{{ $font->slug }}" style="font-family:'{{ $font->slug }}'"><span>{{ $font->name }}</span><small>نمونه متن فارسی</small></label></div>@endforeach</div><small>مدام حالت پیش‌فرض است و برای هر سفارش قابل تغییر است.</small></div>
          ...
          <div class="studio-field"><div class="studio-preview-head"><label>پیش‌نمایش هوشمند قبل از ساخت</label><button class="studio-btn" type="button" id="generate-preview"><i class="fa-solid fa-wand-magic-sparkles"></i> ساخت ۳ پیشنهاد</button></div><small>هوش مصنوعی برای هر بخش سه گزینه می‌سازد؛ گزینهٔ اول انتخاب‌شده است و با کلیک روی هر کارت، گزینهٔ اصلی تغییر می‌کند.</small><div class="studio-preview studio-conditional is-hidden" id="content-preview"><div class="studio-preview-block"><div class="studio-preview-label">هوک</div><div class="studio-preview-options" data-preview-tabs="hook"></div></div><div class="studio-preview-block"><div class="studio-preview-label">کپشن</div><div class="studio-preview-options" data-preview-tabs="caption"></div></div><div class="studio-preview-block"><div class="studio-preview-label">کلمهٔ کلیدی</div><div class="studio-preview-options" data-preview-tabs="keyword"></div></div><div class="studio-preview-status" id="preview-status">آمادهٔ دریافت پیش‌نمایش</div></div></div>
    ...
    <section class="studio-card studio-panel studio-telegram-live" id="telegram-live-preview">
      ...
      <div class="studio-telegram-preview-grid">
        ...
        <div class="studio-telegram-preview-note"><strong>این پیش‌نمایش چه چیزهایی را نشان می‌دهد؟</strong><br>تصاویر انتخاب‌شدهٔ محصول، متن کپشن و دکمه‌های لینک‌دار در همین قاب شبیه‌سازی می‌شوند. با تغییر متن یا تنظیم دکمه‌ها، پیش‌نمایش هم‌زمان به‌روزرسانی می‌شود. انتشار واقعی در کانال بعد از اتصال بات و شناسهٔ موضوع‌ها فعال خواهد شد.</div>
      </div>
    </section>
...
<script>
  ...
  function updateTelegramPreview() {
    ...
    const caption = captionEditor?.value || fallbackCaption || 'متن کپشن تلگرام';
  ...
  document.addEventListener('input', (event) => { if (event.target.closest('#telegram-button-list,[data-preview-tabs="caption"],[name="caption_text"]')) updateTelegramPreview(); });
  ...
  function renderPreviewTabs(kind, values) {
    ...
    (values || []).slice(0, 3).forEach((value, index) => {
      ...
      option.addEventListener('click', (event) => { if (event.target === editor && !editor.readOnly) return; holder.querySelectorAll('.studio-preview-option').forEach((item) => item.classList.remove('is-selected')); option.classList.add('is-selected'); syncPreviewSelection(kind); });
