...
<style>
  ...
  @media(max-width:650px){.video-studio-page{padding:15px}.studio-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.studio-kpi{min-height:108px;padding:12px}.studio-kpi-value{font-size:20px}.studio-health{grid-template-columns:1fr}.studio-prompt-grid{grid-template-columns:1fr}.video-studio-title{font-size:19px}.studio-job-edit-grid{grid-template-columns:1fr}.studio-job-edit-grid .studio-field.full{grid-column:auto}.studio-wizard{padding:5px}.studio-wizard-row{display:grid;grid-template-columns:1fr;gap:3px}.studio-wizard-step{padding:9px}.studio-wizard-connector{width:1px;height:9px;margin-right:24px}.studio-wizard-footer{align-items:stretch;flex-direction:column}.studio-wizard-progress{max-width:none;order:-1}.studio-wizard-nav{justify-content:space-between}.studio-wizard-nav .studio-btn{flex:1;justify-content:center}}
  ...
  /* Final wizard/media contract: switching a tab never changes the shell, and every media choice is a 110px square. */
  .studio-wizard-step.is-active{background:var(--primary-l)!important;box-shadow:0 0 12px -2px color-mix(in srgb,var(--primary) 30%,transparent)!important;border-color:color-mix(in srgb,var(--primary) 28%,transparent)!important}
  #studio-media-controls .studio-option label,#studio-media-controls .studio-choice label{width:110px!important;height:110px!important;min-width:110px;max-width:110px;min-height:110px!important;max-height:110px;aspect-ratio:1/1;box-sizing:border-box;justify-self:center}
  #studio-media-controls #source-options,#studio-media-controls #aspect-ratio-field .studio-options,#studio-media-controls #transition-field .studio-choice-row{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #studio-media-controls #source-options,#studio-media-controls #aspect-ratio-field .studio-options{justify-items:center}
  #studio-media-controls #transition-field .studio-choice-row{display:grid;justify-items:center}
  @media(min-width:1000px){
    #telegram-live-preview .studio-telegram-preview-grid{direction:ltr;grid-template-columns:minmax(280px,.85fr) minmax(0,1.15fr);gap:20px;align-items:start}
    #telegram-live-preview .studio-telegram-preview-grid>*{direction:rtl}
    #telegram-live-preview .studio-phone{justify-self:start}
  ...
  /* فشرده‌سازی گام اول: فونت در کنار سه گروه رسانه‌ای و گزینه‌های ۲×۲ کوچک. */
  .studio-wizard{min-height:0!important}
  #font-family .studio-font-option:first-child{display:none}
  #font-family{grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}
  #font-family .studio-font-option label{min-height:55px;height:55px;padding:5px;font-size:9px;box-sizing:border-box}
  @media(min-width:1000px){
    #studio-settings-form{grid-auto-flow:row dense}
    #studio-settings-form>#font-family-field{grid-column:1!important}
    #studio-settings-form>#studio-media-controls{grid-column:2/span 3!important}
  }
  #studio-media-controls .studio-option label,#studio-media-controls .studio-choice label{width:55px!important;height:55px!important;min-width:55px;max-width:55px;min-height:55px!important;max-height:55px;padding:4px;font-size:8px;box-sizing:border-box}
  #studio-media-controls .studio-option label i{font-size:11px}
  #studio-media-controls #source-options,#studio-media-controls #aspect-ratio-field .studio-options,#studio-media-controls #transition-field .studio-choice-row{grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;justify-items:center}
  ...
  /* استپر سه‌مرحله‌ای هم‌خانواده با صفحهٔ ثبت محصول */
  .studio-wizard{margin-bottom:16px;padding:6px;background:var(--input-bg);border:1px solid var(--border);border-radius:12px}
  .studio-wizard-row{display:flex;align-items:center;gap:0}
  .studio-wizard-step{flex:1;display:flex;align-items:center;gap:10px;min-width:0;padding:11px 10px;border:1px solid transparent;border-radius:10px;cursor:pointer;transition:.2s;text-align:right}
  .studio-wizard-step:hover{background:var(--primary-l)}
  .studio-wizard-step.is-active{background:var(--primary-l);border-color:color-mix(in srgb,var(--primary) 28%,transparent);box-shadow:0 0 12px -2px color-mix(in srgb,var(--primary) 30%,transparent)}
  ...
  .studio-wizard-progress{flex:1;max-width:420px;text-align:center}.studio-wizard-progress-head{display:flex;justify-content:space-between;gap:8px;margin-bottom:5px;font-size:9px;color:var(--text-soft)}.studio-wizard-progress-track{height:7px;overflow:hidden;border:1px solid var(--border);border-radius:99px;background:var(--input-bg)}.studio-wizard-progress-track>span{display:block;width:33.33%;height:100%;border-radius:inherit;background:var(--primary);transition:width .25s}
  .studio-wizard-nav{display:flex;gap:7px}.studio-wizard-nav .studio-btn{height:36px}
  /* ارتفاع و فاصله‌ی استپر مطابق الگوی ثبت محصول */
  .studio-wizard-step{min-height:88px;padding:16px 14px;gap:12px}.studio-wizard-circle{width:48px;height:48px;flex-basis:48px;font-size:14px}.studio-wizard-connector{width:28px;flex-basis:28px;height:2px}
  .studio-panel-head{flex-wrap:wrap}.studio-panel-head .studio-build-actions{margin-inline-start:auto;justify-content:flex-end;max-width:100%;order:initial;width:auto;min-width:0}.studio-panel-head .studio-build-actions .studio-btn{white-space:nowrap}
  ...
  /* آخرین تنظیمات این مرحله: کارت‌ها عرض ثابت دارند و چهار بخش گام اول کنار هم می‌نشینند. */
  #studio-settings{width:100%;min-width:0;box-sizing:border-box}
  #studio-settings-form{width:100%;min-width:0;box-sizing:border-box;row-gap:16px}
  #studio-settings-form>.studio-step-panel,#studio-settings-form>.studio-smart-fields,#studio-settings-form>.studio-media-controls{width:100%;min-width:0;box-sizing:border-box;margin:0}
  #font-family-field>small{display:none}
  #font-family{grid-template-columns:repeat(2,minmax(0,40%));justify-content:center;gap:6px}
  #font-family .studio-font-option label{width:100%;min-height:55px;height:55px;box-sizing:border-box}
  #studio-media-controls .studio-option label,#studio-media-controls .studio-choice label{width:86px!important;height:86px!important;min-width:86px;max-width:86px;min-height:86px!important;max-height:86px;font-size:10px}
  #studio-media-controls .studio-option label i{font-size:14px}
  #studio-media-controls{padding:14px!important;gap:16px!important}
  .studio-media-controls>#source-mode-field,.studio-media-controls>#aspect-ratio-field,.studio-media-controls>#transition-field{padding:16px!important;min-height:0!important}
  .studio-wizard{min-height:0!important;height:auto!important}
  .studio-wizard-row{width:100%;min-width:0}
  .studio-wizard-step{min-width:0;box-sizing:border-box}
  .studio-wizard-footer{margin-top:16px;padding-top:16px}
  .studio-product-field{display:grid;gap:12px}
  .studio-product-field>.studio-selected-product{order:1}
  .studio-product-field>.studio-selected-product-actions{order:2;justify-content:flex-start}
  .studio-product-image-slots{order:3;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;width:100%}
  .studio-product-image-slots .studio-image-choice{min-width:0}
  .studio-product-image-slots .studio-image-choice label,.studio-image-placeholder{display:flex;align-items:center;justify-content:center;aspect-ratio:1;border:2px solid var(--border);border-radius:9px;overflow:hidden;background:var(--input-bg);color:var(--text-soft);text-align:center;font-size:9px}
  ...
  .studio-product-field>small{order:4}
  .studio-text-command{padding:14px;border:1px solid var(--border);border-radius:12px;background:var(--input-bg)}
  #studio-system-layout{margin-top:0}
  .video-studio-page>.studio-grid{order:1}
  .video-studio-page>.studio-settings{order:2}
  .video-studio-page>#studio-system-layout{order:3}
  .video-studio-page>#studio-queue-panel{order:4}
  .video-studio-page>#studio-produced-panel{order:5}
  .video-studio-page>#studio-latest-videos-panel{order:6}
  .video-studio-page>#studio-latest-tests-panel{order:7}
    ...
    <div class="studio-settings" style="margin-bottom:16px">
      <section class="studio-card studio-panel" id="studio-settings">
        ...
        <form id="studio-settings-form" class="studio-form" data-product-id="{{ $selectedProduct?->id ?? (int) request()->query('product_id') }}" method="POST" action="{{ route('admin.video-studio.settings.update') }}" enctype="multipart/form-data">
          ...
          <div class="studio-wizard" role="tablist" aria-label="گام‌های ساخت ویدیو">
            <div class="studio-wizard-row">
              <button class="studio-wizard-step is-active" type="button" data-studio-step-tab="1" role="tab" aria-selected="true"><span class="studio-wizard-circle">۱</span><span class="studio-wizard-copy"><span class="studio-wizard-label">گام اول</span><span class="studio-wizard-title">تنظیمات ساخت ویدیو</span><span class="studio-wizard-desc">محصول، صدا، فونت و ظاهر</span></span></button>
              <span class="studio-wizard-connector" data-studio-connector="1"></span>
              <button class="studio-wizard-step" type="button" data-studio-step-tab="2" role="tab" aria-selected="false"><span class="studio-wizard-circle">۲</span><span class="studio-wizard-copy"><span class="studio-wizard-label">گام دوم</span><span class="studio-wizard-title">تنظیمات ویدیو اینستاگرام</span><span class="studio-wizard-desc">هوک، کپشن و کلمهٔ کلیدی</span></span></button>
              <span class="studio-wizard-connector" data-studio-connector="2"></span>
              <button class="studio-wizard-step" type="button" data-studio-step-tab="3" role="tab" aria-selected="false"><span class="studio-wizard-circle">۳</span><span class="studio-wizard-copy"><span class="studio-wizard-label">گام سوم</span><span class="studio-wizard-title">تنظیمات ویدیو تلگرام</span><span class="studio-wizard-desc">کپشن، دکمه‌ها و پیش‌نمایش</span></span></button>
            </div>
          </div>
          ...
          <div class="studio-field studio-step-panel studio-text-command" data-studio-step="1"><label for="text-command">دستور متنی ساخت یا اصلاح</label><textarea class="studio-textarea" id="text-command" name="text_command" rows="3" placeholder="مثلاً: هوک را کوتاه‌تر و ضخیم‌تر کن، هر دو تصویر محصول را استفاده کن و مدت دعوت به اقدام را کمتر کن."></textarea><small>اگر خروجی نیاز به اصلاح داشت، مشکل را اینجا بنویس تا در ساخت یا بازسازی بعدی اعمال شود.</small></div>
          ...
          <div class="studio-platform-controls studio-step-panel is-hidden" data-studio-step="3"><label class="studio-platform-toggle"><span><i class="fa-brands fa-telegram"></i> خروجی تلگرام</span><input type="hidden" name="telegram_enabled" value="0"><input type="checkbox" name="telegram_enabled" value="1" checked></label></div>
        </form>
      <section class="studio-card studio-panel studio-telegram-live studio-step-panel is-hidden" data-studio-step="3" id="telegram-live-preview">
        ...
        <div class="studio-wizard-footer" aria-label="ناوبری گام‌های ساخت">
          <div class="studio-wizard-nav"><button class="studio-btn" type="button" id="studio-step-prev" disabled><i class="fa-solid fa-arrow-right"></i> مرحلهٔ قبل</button><button class="studio-btn primary" type="button" id="studio-step-next">مرحلهٔ بعد <i class="fa-solid fa-arrow-left"></i></button></div>
          <div class="studio-wizard-progress"><div class="studio-wizard-progress-head"><span>پیشرفت تنظیمات</span><strong id="studio-step-progress-label">گام ۱ از ۳</strong></div><div class="studio-wizard-progress-track"><span id="studio-step-progress-fill"></span></div></div>
        </div>
      </section>
    </div>
      ...
      <div class="studio-modal-card">
        ...
        <form id="job-edit-form" class="studio-job-edit-form" method="POST" enctype="multipart/form-data">
          ...
          <div class="studio-job-edit-grid">
            ...
            <div class="studio-field full"><label for="job-edit-text-command">دستور متنی اصلاحات</label><textarea class="studio-textarea" id="job-edit-text-command" name="text_command" rows="3" placeholder="اصلاحات را محاوره‌ای بنویس..."></textarea></div>
          ...
          @foreach($jobs as $job)
            ...
            @php
              $jobEditorConfig = [
                ...
                'text_command' => data_get($job->payload, 'text_command', ''),
              ];
            @endphp
            <div class="studio-job">
              ...
              <div class="studio-muted">#{{ $job->id }} · کد {{ $job->shortCode() }}</div>
...
<script>
  ...
  document.querySelectorAll('#open-prompt-mother,#open-prompt-mother-top,#open-prompt-telegram').forEach((button) => button.addEventListener('click', () => promptMotherModal?.classList.add('is-open')));
  ...
  function setStudioStep(step, shouldScroll = false) {
    ...
    document.querySelectorAll('[data-studio-step-tab]').forEach((tab) => {
      const isActive = tab.dataset.studioStepTab === String(activeStudioStep);
      const isComplete = Number(tab.dataset.studioStepTab) < activeStudioStep;
      tab.classList.toggle('is-active', isActive);
      tab.classList.toggle('is-complete', isComplete);
      tab.setAttribute('aria-selected', isActive ? 'true' : 'false');
      const circle = tab.querySelector('.studio-wizard-circle');
      if (circle) circle.innerHTML = isComplete ? '<i class="fa-solid fa-check"></i>' : studioStepFa(tab.dataset.studioStepTab);
    });
    document.querySelectorAll('[data-studio-connector]').forEach((connector) => connector.classList.toggle('is-complete', Number(connector.dataset.studioConnector) < activeStudioStep));
    const previous = document.getElementById('studio-step-prev');
    const next = document.getElementById('studio-step-next');
    if (previous) previous.disabled = activeStudioStep === 1;
    if (next) { next.disabled = activeStudioStep === 3; next.classList.toggle('opacity-50', activeStudioStep === 3); next.innerHTML = activeStudioStep === 3 ? 'آخرین گام <i class="fa-solid fa-check"></i>' : 'مرحلهٔ بعد <i class="fa-solid fa-arrow-left"></i>'; }
    const fill = document.getElementById('studio-step-progress-fill');
    if (fill) fill.style.width = ((activeStudioStep / 3) * 100) + '%';
