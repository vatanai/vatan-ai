    ...
    <div class="studio-settings" style="margin-bottom:16px">
      <section class="studio-card studio-panel" id="studio-settings">
        ...
        <form id="studio-settings-form" class="studio-form" method="POST" action="{{ route('admin.video-studio.settings.update') }}" enctype="multipart/form-data">
          ...
          @endif
          <input type="hidden" name="font_family" value="B_Yekan"><input type="hidden" name="build_now" id="build-now" value="0">
          ...
          <div class="studio-field"><label for="prompt-profile">پروفایل یادگیری هوش مصنوعی</label><textarea id="prompt-profile" class="studio-textarea" name="prompt_profile" rows="8" placeholder="قواعد ثابت تولید هوک، کپشن و کلمهٔ کلیدی را اینجا بنویس یا فایل پرامپت بارگذاری کن.">{{ old('prompt_profile', $settings->prompt_profile) }}</textarea><input class="studio-input" type="file" name="prompt_file" accept=".txt,.md,text/plain,text/markdown"><small>این پروفایل همراه هر سفارش به مدل ارسال می‌شود. نمونه‌های خوب را در کتابخانهٔ هوک ثبت کن تا در اصلاحات بعدی از آن‌ها الگو بگیرد.</small></div>
