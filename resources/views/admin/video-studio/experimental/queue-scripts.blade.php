...
(() => {
  ...
  const render = payload => {
    ...
    rows.forEach(job => {
      ...
      const statusCell = document.createElement('td'); const cost = document.createElement('span'); cost.className = 'v2-job-cost'; const hasCost = job.cost_toman !== null && job.cost_toman !== undefined && Number.isFinite(Number(job.cost_toman)); cost.textContent = hasCost ? `هزینهٔ تقریبی: ${Number(job.cost_toman).toLocaleString('fa-IR')} تومان` : 'نرخ مدل ثبت نشده'; const status = document.createElement('small'); status.className = 'v2-job-status'; status.textContent = statusLabel(job.status); statusCell.append(cost, status);
