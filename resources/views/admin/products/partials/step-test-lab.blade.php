...
function renderTestResultCard(card,data,model){if(!data.success){card.innerHTML='<div class="aspect-square flex items-center justify-center p-5 text-center text-[11px] text-[var(--danger)]">'+escapeTestHtml(data.message||'اجرای مدل ناموفق بود')+'</div><div class="p-3 text-[10px] font-mono text-[var(--text3)] truncate" dir="ltr">'+escapeTestHtml(model)+'</div>';return;}var tokens=data.usage?.total_tokens;card.dataset.runId=data.run_id;card.innerHTML='<a href="'+data.image_url+'" target="_blank" class="block aspect-square bg-[var(--bg)]"><img src="'+data.image_url+'" class="w-full h-full object-contain" alt="خروجی آزمایش"></a><div class="p-3"><div class="text-[10px] font-mono text-[var(--text2)] truncate mb-2" dir="ltr">'+escapeTestHtml(model)+'</div><div class="grid grid-cols-2 gap-2 mb-3"><div><small class="block text-[9px] text-[var(--text3)]">زمان اجرا</small><strong class="text-[11px] text-[var(--text)]">'+formatTestDuration(data.duration_ms)+'</strong></div><div><small class="block text-[9px] text-[var(--text3)]">مصرف اعتبار</small><strong class="text-[11px] text-[var(--text)]">'+(tokens==null?'اعلام نشده':Number(tokens).toLocaleString('fa-IR'))+'</strong></div></div>'+testFeedbackHtml(data.run_id,null,'',false)+'</div>';wireTestFeedback(card);}
