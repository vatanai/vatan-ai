        ...
        <div class="bg-s1 border border-b1 rounded-[10px] p-5 max-[768px]:p-[18px] max-[480px]:p-[14px]">
          ...
          <div class="overflow-x-auto">
            <table class="w-full border-collapse">
              <thead>
                <tr>
                  <th class="text-[10px] font-bold text-watan-text3 text-right pb-[10px] px-3 border-b border-b1 whitespace-nowrap">اجراکننده</th>
                  <th class="text-[10px] font-bold text-watan-text3 text-right pb-[10px] px-3 border-b border-b1 whitespace-nowrap">محصول / منبع</th>
              ...
              <tbody>
                @forelse(($creditTransactions ?? collect()) as $transaction)
                  @php($statusClass = $transaction['is_success'] ? 'bg-green/[0.08] text-green border-green/[0.2]' : (in_array($transaction['status_key'], ['failed'], true) ? 'bg-red/[0.1] text-red border-red/[0.2]' : 'bg-orange/[0.1] text-orange border-orange/[0.25]'))
                  <tr class="group"><td class="group-hover:bg-s2 text-xs text-watan-text2 py-[10px] px-3 border-b border-b1 whitespace-nowrap align-middle"><strong class="text-watan-text">{{ $transaction['actor_label'] }}</strong><br><span class="text-[10px] text-watan-text3">{{ $transaction['user_name'] }}</span></td><td class="group-hover:bg-s2 text-xs text-watan-text2 py-[10px] px-3 border-b border-b1 whitespace-nowrap align-middle"><strong class="text-watan-text">{{ $transaction['product_name'] }}</strong><br><span class="text-[10px] text-watan-text3">{{ $transaction['source_label'] }} · {{ $transaction['provider'] }}</span></td><td class="group-hover:bg-s2 text-watan-text font-bold py-[10px] px-3 border-b border-b1 whitespace-nowrap align-middle">{{ $transaction['amount_toman'] !== null ? number_format($transaction['amount_toman']).' ت' : '—' }}</td><td class="group-hover:bg-s2 py-[10px] px-3 border-b border-b1 whitespace-nowrap align-middle"><span class="text-[10px] font-bold py-[2px] px-2 rounded-md inline-block {{ $statusClass }}">{{ $transaction['status_label'] }}</span></td><td class="group-hover:bg-s2 text-xs text-watan-text2 py-[10px] px-3 border-b border-b1 whitespace-nowrap align-middle">{{ $transaction['date_jalali'] }}</td></tr>
                @empty
                  <tr><td colspan="5" class="text-center text-xs text-watan-text3 py-6">هنوز رخداد مصرفی ثبت نشده است.</td></tr>
                @endforelse
              </tbody>
            </table>
          </div>
