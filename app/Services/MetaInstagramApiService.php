...
class MetaInstagramApiService
{
    public function testConnection(MarketingIntegration $integration): array
    {
        ...
        try {
            $profileResponse = $this->client($token)->get('/'.$instagramUserId, [
            ...
            $mediaResponse = $this->client($token)->get('/'.$instagramUserId.'/media', [
    ...
    private function sendMessage(MarketingIntegration $integration, array $payload): array
    {
        ...
        try {
            $response = $this->client($token)->post('/'.$instagramUserId.'/messages', $payload);
    ...
    {
        return Http::baseUrl(rtrim((string) config('services.meta.graph_url', 'https://graph.instagram.com'), '/').'/'.trim((string) config('services.meta.graph_version', 'v24.0'), '/'))
            ->withToken($token)
            ->acceptJson()
            ->connectTimeout(10)
            ->timeout(20);
